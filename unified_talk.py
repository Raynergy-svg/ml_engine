...
def _load_df_from_oanda(
    ...
    *,
    ...
    if load_dotenv is not None:
        # Load .env from cwd when present; no-op otherwise.
        load_dotenv()
...
def _print_talk_banner(ctx: TalkContext) -> None:
    ...
    print("Commands: 'use csv <path>' | 'use ticker <SYM>' | 'use oanda [INSTR GRAN COUNT]' | 'predict' | 'help' | 'exit'")
    print("Tip: after you run 'predict' once, you can ask natural questions like 'should I buy?' or 'what's the trend?'.")
...
def _answer_from_last(ctx: TalkContext, q: str) -> str:
    if ctx.last_result is None:
        return "I don't have a recent prediction yet. Run 'predict' (or 'predict csv ...' / 'predict ticker ...') first."
...
def _handle_basic_commands(ctx: TalkContext, ql: str) -> Optional[bool]:
    ...
    if ql in {"help", "?"}:
        print(
            "Assistant: Try 'use csv market_data/MSFT_data.csv' then 'predict'. Or 'use ticker MSFT' then 'predict'.\n"
            "Assistant: OANDA demo/practice: 'use oanda EUR_USD M5 300' then 'predict'.\n"
            "Assistant: Trading (practice): 'trade buy 1000 EUR_USD' (dry-run unless 'execute on').\n"
            "Assistant: After a prediction, ask questions like 'should I buy?', 'what is the risk?', 'what's the trend?'."
        )
        return True

    if ql in {"execute on", "execute true"}:
        ctx.oanda_execute = True
        print("Assistant: execute mode ON (practice orders will be sent).")
        return True
    if ql in {"execute off", "execute false"}:
        ctx.oanda_execute = False
        print("Assistant: execute mode OFF (dry-run).")
        return True
...
def _handle_use_commands(ctx: TalkContext, q: str, *, period: str, interval: str) -> Optional[bool]:
    ...
    if use_csv_arg is not None:
        ...
        print(f"Assistant: loaded {ctx.active_source}. Run 'predict' or ask a question.")
        return True
...
def _handle_predict_commands(ctx: TalkContext, q: str, ql: str, *, period: str, interval: str) -> Optional[bool]:
    ...
    if ql == "predict":
        if ctx.active_df is None:
            print("Assistant: pick a source first: 'use csv <path>' or 'use ticker <SYM>'.")
            return True
...
def _handle_oanda_info_commands(ctx: TalkContext, q: str, ql: str) -> Optional[bool]:
    if ql.startswith("oanda quote"):
        ...
        print(f"Assistant: {instrument} bid={quote['bid']:.6f} ask={quote['ask']:.6f}")
        return True
    ...
    if ql in {"oanda account", "oanda summary"}:
        ...
        print(f"Assistant: account NAV={nav} balance={bal} marginAvailable={margin}")
        return True
...
def _handle_trade_close(ctx: TalkContext, parts: list[str]) -> None:
    ...
    if not ctx.oanda_execute:
        print(f"Assistant: dry-run: would close position for {instrument}. (run 'execute on' to send)")
        return
    ctx.oanda_client.close_position(instrument=instrument)
    print(f"Assistant: close_position sent for {instrument}.")
...
def _handle_trade_market(ctx: TalkContext, *, action: str, parts: list[str]) -> None:
    ...
    if not ctx.oanda_execute:
        print(
            f"Assistant: dry-run: would place MARKET order {action.upper()} {abs(signed_units)} {instrument}. "
    ...
    print(f"Assistant: order submitted (id={tx.get('id')}).")
...
def _handle_talk_command(ctx: TalkContext, q: str, *, period: str, interval: str) -> bool:
    ...
    print(ASSISTANT_PREFIX + _answer_from_last(ctx, q))
    return True

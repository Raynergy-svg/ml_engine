...
class CrossPairValidator:
    ...
    def evaluate_pair(self, pair: str) -> Dict:
        ...
        try:
            ...
            if collapse:
                console.print("  ⚠️ PREDICTION COLLAPSE DETECTED")
    ...
    for path in model_paths:
        if path and os.path.exists(path):
            try:
                ...
                console.print("  ⚠️ Standard load failed, trying compile=False...")
                try:
            ...
            try:
                ...
                if 'scaler' in meta and meta['scaler'] is not None:
                    ...
                    if hasattr(scaler, 'mean_') or hasattr(scaler, 'center_'):
                        console.print("     ✅ Scaler is properly fitted")
                    else:
                        console.print("     ⚠️ Scaler may not be fitted")
                    break
            except Exception as e:
            ...
            if path and os.path.exists(path):
                ...
                if hasattr(scaler, 'center_'):
                    ...
                    if np.allclose(scaler.center_, 0) and np.allclose(scaler.scale_, 1):
                        console.print("     ⚠️ WARNING: Scaler is identity transform - may need retraining!")
                break

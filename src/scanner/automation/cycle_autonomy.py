...
class CycleAutonomyTriggers:
    ...
    def on_cycle_complete(
        ...
        try:
            ...
            # US-006: Poll soak orchestrator status. Poll the SAME module
            # singleton that .trigger() used (autonomous_trainer
            # ._get_soak_orchestrator) — a fresh SoakOrchestrator() has
            # _process=None and can never observe 'running'/'complete'.
            _soak_status = _at._get_soak_orchestrator().poll()

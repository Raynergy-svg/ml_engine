"""Producer-side crypto-momentum evaluation worker (authority-free).

Given a signed ``JobManifest``, the matching signed ``DatasetManifest`` and
``StrategyManifest``, and the declared per-fold partition bytes, the worker:
...
4. scores every fold and aggregates one head per universe construction with the injected
   evaluator (which refuses an incomplete/duplicate fold set),
5. requires every declared construction head to be produced,
6. packages each construction into an immutable ``EvidencePackage`` with its own
...
from .models import CryptoMomentumWorkerOutput, PackagedConstructionHead, ConstructionHeadResult

logger = logging.getLogger(__name__)

Evaluator = Callable[..., "tuple[ConstructionHeadResult, ...]"]
...
class MissingHeadError(ValueError):
    """A declared construction head was not produced (aggregation is incomplete)."""
...
def _fail_open_registry(experiment: str, metrics: Mapping[str, float]) -> None:
    ...
    try:  # pragma: no cover - exercised via injection in tests
        ...
        mirror_training_session(
            {"pair": "crypto_momentum", "timestamp": experiment, "granularity": "cell",
             "warm_start": False, "duration_seconds": 0.0,
             "models_trained": ["momentum_scorecard"], "hyperparams": {}, "metrics": dict(metrics)},
            experiment_name=experiment,
        )
    except Exception as exc:  # noqa: BLE001 - fail-open by contract
        logger.warning("crypto_momentum registry mirror failed (ignored): %s", exc)
...
def _build_evaluation_report(
    ...
    *,
    ...
    return EvaluationReport(
        report_id=f"crypto-momentum-{head.campaign_id}-{head.construction}-eval",
        ...
        trial_count=1,
        effective_sample_size=float(head.n_folds),
        metrics=metrics,
        gates=head.gates,
...
def _package_head(
    ...
    *,
    ...
    artifact_path = "artifacts/construction_scorecard.json"
    artifact = ArtifactRef(
        artifact_id=f"crypto-momentum-{head.campaign_id}-{head.construction}-scorecard",
    ...
    if head.strategy_return_bytes:
        ...
        contract_artifact = ArtifactRef(
            artifact_id=f"crypto-momentum-{head.campaign_id}-{head.construction}-strategy-returns",
    ...
    package = EvidencePackage(
        package_id=f"crypto-momentum-{head.campaign_id}-{head.construction}",
    ...
    created_event = DispositionEvent(
        ...
        reason=f"remote worker produced {head.construction} construction evidence for {head.campaign_id}",
        metadata={"campaign_id": head.campaign_id, "construction": head.construction,
                  "verdict": head.verdict, "passed": head.passed},
    )
    created_envelope = producer.sign(created_event, created_at=created_at)

    return PackagedConstructionHead(
...
def run_worker(
    *,
    ...
    """Execute one signed crypto-momentum job and emit per-construction evidence."""
    ...
    produced: dict[str, ConstructionHeadResult] = {}
    for head in results:
        if head.lane_id in produced:
            raise MissingHeadError(f"duplicate construction lane {head.lane_id!r} produced")
        produced[head.lane_id] = head
    expected = set(job.expected_outputs)
    if set(produced) != expected:
        raise MissingHeadError(
            f"produced construction lanes {sorted(produced)} do not match declared outputs {sorted(expected)}"
        )
    ...
    for head in packaged:
        try:
            ...
            logger.warning("crypto_momentum registry publish failed (ignored): %s", exc)
    ...
    return CryptoMomentumWorkerOutput(

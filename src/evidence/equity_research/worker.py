"""Producer-side equity-research evaluation worker (authority-free).
...
4. scores every fold and aggregates one head per universe tier with the injected
   evaluator (which refuses an incomplete/duplicate fold set),
5. requires every declared tier head to be produced,
6. packages each tier into an immutable ``EvidencePackage`` with its own
...
from .models import EquityResearchWorkerOutput, PackagedTierHead, TierHeadResult

logger = logging.getLogger(__name__)

Evaluator = Callable[..., "tuple[TierHeadResult, ...]"]
...
class MissingHeadError(ValueError):
    """A declared tier head was not produced (aggregation is incomplete)."""
...
def _fail_open_registry(experiment: str, metrics: Mapping[str, float]) -> None:
    ...
    try:  # pragma: no cover - exercised via injection in tests
        ...
        mirror_training_session(
            {"pair": "equity_research", "timestamp": experiment, "granularity": "fold",
             "warm_start": False, "duration_seconds": 0.0,
             "models_trained": ["rank_ic_scorecard"], "hyperparams": {}, "metrics": dict(metrics)},
            experiment_name=experiment,
        )
    except Exception as exc:  # noqa: BLE001 - fail-open by contract
        logger.warning("equity_research registry mirror failed (ignored): %s", exc)
...
def _verify_partition_hashes(
    ...
    for partition_id, data in partitions.items():
        ...
        if len(data) != ref.size_bytes or sha256_bytes(data) != ref.digest:
            raise DatasetHashError(f"fold partition {partition_id!r} does not match its declared hash")
...
def _build_evaluation_report(
    ...
    *,
    ...
    return EvaluationReport(
        report_id=f"equity-research-{head.hypothesis_id}-{head.tier}-eval",
...
def _package_head(
    ...
    *,
    ...
    artifact_path = "artifacts/tier_scorecard.json"
    artifact = ArtifactRef(
        artifact_id=f"equity-research-{head.hypothesis_id}-{head.tier}-scorecard",
    ...
    package = EvidencePackage(
        package_id=f"equity-research-{head.hypothesis_id}-{head.tier}",
    ...
    created_event = DispositionEvent(
        ...
        reason=f"remote worker produced {head.tier} tier evidence for {head.hypothesis_id}",
        metadata={"hypothesis_id": head.hypothesis_id, "tier": head.tier,
                  "verdict": head.verdict, "passed": head.passed},
    )
    created_envelope = producer.sign(created_event, created_at=created_at)

    return PackagedTierHead(
...
def run_worker(
    *,
    ...
    """Execute one signed equity-research job and emit per-tier evidence."""
    ...
    results = evaluator(partitions, params, hypothesis_id=hypothesis_id)
    produced = {head.lane_id: head for head in results}
    expected = set(job.expected_outputs)
    if set(produced) != expected:
        raise MissingHeadError(
            f"produced tier lanes {sorted(produced)} do not match declared outputs {sorted(expected)}"
        )
    ...
    for head in packaged:
        try:
            ...
            logger.warning("equity_research registry publish failed (ignored): %s", exc)
    ...
    return EquityResearchWorkerOutput(

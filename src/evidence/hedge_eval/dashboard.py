"""Read-only hedge-evaluation evidence cockpit view.

Deliberately dependency-free (stdlib ``json`` + ``pathlib`` only): the dashboard
must be able to render the evidence state without importing the hedge/scoring
stack or any signing keys. It reads the store's own committed
...
def hedge_evidence_view(store_root: str | Path) -> dict:
    """Build a per-strategy display view from the store's committed projection."""

"""End-to-end hedge-evaluation evidence vertical slice + read-only cockpit view.

``run_hedge_evidence_slice`` executes the whole roadmap §12 flow for one signed
job over the raw-vs-hedged shadow ledger: it builds and signs the dataset /
strategy / capability / job manifests, runs the authority-free worker, and
imports every produced strategy head into the local evidence store, ending each
at QUARANTINED or REJECTED. A candidate never overwrites an incumbent — the
slice never promotes a champion.

``hedge_evidence_view`` is a display-only reader over the store's own committed
index projection (no signing keys required), suitable for the dashboard.
"""
...
from .dashboard import hedge_evidence_view
...
from .models import HedgeImportOutcome, HedgeWorkerOutput
...
# logic in src/hedge/hedge_scorecard.py that would change reproduced numbers).
HEDGE_SCORECARD_VERSION = "hedge_scorecard.v1"
...
_DEFAULT_ACTORS = {
    AuthorityRole.PRODUCER: "hedge-eval-remote-worker",
...
class Produced:
    ...
    worker_output: HedgeWorkerOutput
...
class SliceResult:
    ...
    worker_output: HedgeWorkerOutput
    outcomes: Mapping[str, HedgeImportOutcome] = field(default_factory=dict)
...
def produce_worker_output(
    ...
    *,
    ...
    created_at: datetime,
    git_commit: str,
    job_id: str = "hedge-eval-vertical-slice",
    ...
    strategies = sorted(partitions)

    capability_profile = build_capability_profile()
    capability_envelope = producer.sign(capability_profile, created_at=created_at)

    dataset_manifest = build_hedge_ledger_dataset_manifest(
        partitions,
        dataset_id=dataset_id,
        retrieved_at=retrieved_at,
        coverage_start=coverage_start,
        coverage_end=coverage_end,
        row_counts={s: data.count(b"\n") for s, data in partitions.items()},
    )
    dataset_envelope = producer.sign(dataset_manifest, created_at=created_at)

    strategy_manifest = build_hedge_strategy_manifest(
        strategies, frozen_at=created_at, min_history=params.min_history,
    )
    strategy_envelope = producer.sign(strategy_manifest, created_at=created_at)

    container_digest = sha256_bytes(b"axiom-hedge-eval:local-inprocess:v1")
    job = build_hedge_evaluation_job_manifest(
        job_id=job_id,
        ...
        configuration_digest=_config_digest(params, HEDGE_SCORECARD_VERSION),
        scorecard_version=HEDGE_SCORECARD_VERSION,
    ...
    return Produced(
        job=job, strategy_manifest=strategy_manifest,
...
def import_worker_output(
    ...
    *,
    ...
    params: EvaluationParams | None = None,
    evaluator=evaluate_partitions,
) -> dict[str, HedgeImportOutcome]:
    """Import every produced strategy head (QUARANTINED/REJECTED)."""
    params = params or EvaluationParams()
    outcomes: dict[str, HedgeImportOutcome] = {}
...
def run_hedge_evidence_slice(
    ...
    *,
    ...
    """Run the full ledger -> job -> worker -> import flow for one job."""
    params = params or EvaluationParams()

"""Structural no-authority boundary for the portfolio-book evidence worker.

Roadmap §14 hard rule: "No lane becomes capital-active solely from a
standalone gate" — operationalized here the same way every other AXIOM
evidence slice's authority boundary is: as deterministic, re-runnable checks
(per L-005, "advisory != enforced"), not docstring promises.

1. AST import scan — the producer-side modules import nothing that can reach
   the broker path, the trade execution/gate path, or the state engine.
...
4. Capability contract — the profile the worker runs under grants no
   authority and cannot write a champion pointer or approve evidence.
...
from src.evidence.portfolio_book.manifests import build_capability_profile
...
WORKER_MODULES = [
    "src/evidence/portfolio_book/worker.py",
    "src/evidence/portfolio_book/evaluation.py",
    "src/evidence/portfolio_book/manifests.py",
    "src/evidence/portfolio_book/models.py",
]

FORBIDDEN_IMPORT_SUBSTRINGS = [
    "src.scanner", "src.brokers", "scanner.execution", "scanner.gates",
    "scanner.state_engine", "brokers.factory", "oanda_practice", "oanda.client", "state_engine",
]

FORBIDDEN_OPERATIVE_LITERALS = [
    ".claude/state.json", "state.json", "oanda_environment", "OANDA_API_KEY",
    "OANDA_TOKEN", "api-fxpractice", "stream-fxpractice", "trained_data/models",
]
...
def test_worker_modules_contain_no_control_or_credential_literals():
    violations = []
    for rel_path in WORKER_MODULES:
        for value in _code_string_constants(REPO_ROOT / rel_path):
            lowered = value.lower()
...
def test_worker_transitive_import_closure_is_authority_free():
    ...
    code = (
        ...
        "importlib.import_module('src.evidence.portfolio_book.worker')\n"
        "importlib.import_module('src.evidence.portfolio_book.evaluation')\n"
        "importlib.import_module('src.evidence.portfolio_book.manifests')\n"
...
def test_capability_profile_grants_no_authority():
    ...
    assert all("sleeve_returns" in scope for scope in profile.read_scopes)

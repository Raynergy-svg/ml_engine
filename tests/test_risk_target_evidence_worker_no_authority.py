"""Structural no-authority boundary for the risk-target evidence worker.

Roadmap §12 acceptance criteria:
    - Remote worker cannot read `.claude/state.json`.
    - Remote worker has no broker credentials.

These are operationalized here as deterministic, re-runnable checks (per L-005,
"advisory != enforced"), not docstring promises:
...
3. Capability contract — the profile the worker runs under grants no authority
   (every ``may_*`` flag is ``False`` and there are no network endpoints).
...
WORKER_MODULES = [
    "src/evidence/risk_target/worker.py",
    "src/evidence/risk_target/evaluation.py",
    "src/evidence/risk_target/manifests.py",
    "src/evidence/risk_target/models.py",
]
...
# explain the boundary, are excluded) so an explanatory comment about "the
# broker path" can never trip the guard while a real `open(".claude/state.json")`
# would.
...
def test_worker_transitive_import_closure_is_authority_free():
    ...
    contains no broker / execution / gate / state-engine module. This catches
    a future regression where a worker dep pulls one in *through* src.training.*
    (security review B / code review F3, 2026-07-13)."""
    forbidden = [
    ...
    code = (
        ...
        "importlib.import_module('src.evidence.risk_target.worker')\n"
        "importlib.import_module('src.evidence.risk_target.evaluation')\n"
        "importlib.import_module('src.evidence.risk_target.manifests')\n"

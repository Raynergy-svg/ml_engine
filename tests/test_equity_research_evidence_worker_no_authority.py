"""Structural no-authority boundary for the equity-research evidence worker.
...
3. Transitive import closure — importing the producer modules in a fresh
   ...
   *through* their dependency on the analysis-only rank-IC scorecard.
...
from src.evidence.equity_research.manifests import build_capability_profile
...
WORKER_MODULES = [
    "src/evidence/equity_research/worker.py",
    "src/evidence/equity_research/evaluation.py",
    "src/evidence/equity_research/manifests.py",
    "src/evidence/equity_research/models.py",
    "src/equity/research/rank_ic_scorecard.py",
]
...
def test_worker_transitive_import_closure_is_authority_free():
    ...
    future regression where a worker dep pulls one in *through* the rank-IC
    ...
    code = (
        ...
        "importlib.import_module('src.evidence.equity_research.worker')\n"
        "importlib.import_module('src.evidence.equity_research.evaluation')\n"
        "importlib.import_module('src.evidence.equity_research.manifests')\n"

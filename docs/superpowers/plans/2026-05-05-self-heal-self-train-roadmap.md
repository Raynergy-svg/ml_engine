...
> **Active step:** ➜ **C1.A patch SHIPPED + smoke-validated PARTIAL.** Patch is structurally correct (EUR_USD scaler now saved; M15 holdout +9.5pp, H4 +6.7pp, aggregate +5pp). H1 specifically didn't benefit — promotion gate (primary-TF ≥52%) blocked. Joint dir untouched (HOLD verdict). **OPERATOR DECISION blocking next move:** run full 17-pair sweep / investigate H1-specific issue / accept partial fix + close. Heuristic bridges + tests still on main from prior tick; consumer wiring still deferred. Main HEAD `c985562`.
> **Last updated:** 2026-05-07 — C1.A patch + smoke retrain wrap. Halt=true throughout; 52% threshold untouched.

...
> **Active step:** ➜ A1.5 — populate per-pair confidence pkls via `scripts/retrain_per_pair_confidence.py` (then A2 W&B control plane). A1 closed via FF (13 commits landed on main, op A path; Q2/Q3 deferred).
> **Last updated:** 2026-05-06 (A1 closed; main HEAD at 77b70a7; per-pair training code is live, per-pair pkls need fresh retrain on current main journal).
...
| **A1.Q1** — fast-forward 13/15 to main NOW, or hold A1 until per-pair fine-tunes (`a5f5f02`) also land? | A1 closure | trade-off: ff now replaces fake-signal-leak (R²=0.99) with honest-noise (R²=-0.01) — epistemically correct but gates.py confidence input becomes near-random until per-pair lands; hold means leak-fixed code stays on feature branch, main keeps shipping the leak version. **Recommendation:** ff now if joint confidence input is not load-bearing for live decisions (verify gates.py routing); hold otherwise. |
| **A1.Q2** — re-pick `e63abe9` script-only and re-run `scripts/backfill_ridge_features.py` against current main journal? | A1 closure | recommend yes — script + tests are clean code adds; journal divergence is real new data. Re-running backfill produces an honest fresh artifact instead of a stale one. |
| **A1.Q3** — re-pick `a5f5f02` binary overwrite of main's USD_JPY.pkl? | A1 closure | needs verification: is main's USD_JPY.pkl from a stale-pre-fix retrain (overwrite is correct) or a newer artifact (overwrite would clobber)? Compare `git log -- trained_data/models/USD_JPY/ridge_confidence.pkl` on main vs `a5f5f02` to date the artifacts. |

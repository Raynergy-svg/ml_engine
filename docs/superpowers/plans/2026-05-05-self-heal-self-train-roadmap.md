...
> **Active step:** ➜ **VALIDATION PIPELINE BROKEN — every M15 holdout number tonight was illusory.** The trained transformer was never actually evaluated; the "70%" was the all-SHORT fallback predictor's accuracy on whatever the test slice's class balance happened to be. Read `docs/superpowers/plans/2026-05-08-ml-validation-broken.md` FIRST. 3-bug chain: Bug 1 (holdout 1-row vs 60-bar) FIXED; Bug 2 (per-pair routing default-off) PARTIAL; Bug 3 (feature schema drift) PARTIAL — operator chooses 3.A / 3.B / 3.C. Pair-expansion paused; bot stays halted (correctly — inference path is broken). Main HEAD `fde458a`.
> **Last updated:** 2026-05-08 (overnight autonomous run) — 3-bug chain in validation pipeline diagnosed; Bug 1 fixed; Bug 2 partial; Bug 3 partial (time features fixed, regime augmentation still missing). Honest accounting of what shipped vs what's actually validated in the new CHECKPOINT LOG entry "2026-05-08 04:35".
...
### 2026-05-08 04:35 UTC Claude — VALIDATION PIPELINE BROKEN — every M15 holdout number tonight was illusory
- did: per operator's "iterate until morning, take accountability and decisions" mandate, pursued the goal of validating all 15 pairs at M15. Discovered, in escalating order, **THREE compounding bugs** that compose into "every M15 holdout number we shipped tonight (70.0% / 70.2% / 72.2% / 70.5% / etc) was the all-SHORT fallback predictor's accuracy on whatever the test slice's class balance happened to be — the trained transformer was never actually evaluated."
  1. **Bug 1 (FIXED, commit `deedf76`):** `scripts/scheduled_retrain.py:452` and `scripts/per_pair_holdout_eval.py` passed single-row DataFrames to `evaluate_all_gates`. `evaluate_transformer` at `src/scanner/gates.py:1629` requires `seq_len=60`; 1 row triggered the early-return; gate fell back to `momentum > 0.5 ? "LONG" : "SHORT"` which with default-near-0 momentum was always SHORT. Fixed to pass `iloc[i-SEQ_LEN+1:i+1]` rolling 60-bar windows.
  2. **Bug 2 (PARTIAL, explicit flag now used):** `GateEvaluator(...)` defaults to `use_per_pair_routing=False`. CLAUDE.md design says auto-enabled when per-pair dirs exist; verified probe shows it does NOT auto-enable. Even with Bug 1 fixed, eval was using the scaler-null joint model instead of C1.A-fixed per-pair models. Fixed in `per_pair_holdout_eval.py` via explicit `use_per_pair_routing=True`. Same bug likely affects production scanner — need to verify it explicitly enables routing at runtime.
  3. **Bug 3 (DIAGNOSED + PARTIAL):** trained EUR_USD model expects 50 features selected from 64-column training matrix (`max(selected_indices)=63`). Current `compute_normalized_features` at inference produces 58 columns. Schema drift breakdown:
     - 11 features in trained model but missing from current pipeline:
       - `dow_cos, dow_sin, hour_cos, hour_sin` (cyclical time encoding)
       - `regime_extreme, regime_high, regime_low, regime_normal` (TCN-derived regime one-hot)
       - `session_london, session_ny, session_overlap` (FX session one-hot)
     - 19 features in current pipeline but NOT in trained model: raw OHLCV + return variants + RSI variants + wick ratios
     - **Sub-bug 3.1 FIXED (commit `fde458a`)**: `compute_normalized_features:1029` used `time_col.hour.values` which fails when `time_col` is a Series (vs DatetimeIndex); needs `.dt.hour` for Series. The catch silently dropped 7 of 11 features at inference. Fixed; column count went 58→65 post-fix.
     - **Sub-bug 3.2 OPEN**: regime one-hot features are added inside `load_direction_data:1925-1933` (training-time augmentation), NOT in `compute_normalized_features`. Inference path has no equivalent augmentation, so 4 regime features are still missing. Even with 7 of 11 restored, model.predict() still rejects (column-order or count still mismatched).
     - **Sub-bug 3.3 OPEN**: `selected_indices` references training-time column ORDER. Restoring features without preserving order gives the model a different 50-feature subset than it was trained on. Predictions would be technically valid but semantically wrong.
- Three fix options for Bug 3 fully open (3.A restore regime augmentation in inference / 3.B retrain everything with current 58-col schema / 3.C subset+reorder+zero-pad to align inference to saved feature_names). Recommended 3.A; needs operator decision since touches inference pipeline architecture.
- pair-expansion goal "all 15 must pass" PAUSED. Continuing to retrain while eval is broken just produces more fake holdout numbers. Halted retrain iteration after Bug 3 diagnosis surfaced.
- commits this run: `deedf76` (Bug 1 holdout 1-row→60-bar), `a1cfc7f` (validation-broken doc), `7b688e4` (Bug 3 root cause), `fde458a` (Bug 3.1 time-extraction). All non-destructive; halt=true throughout; bot remained correctly halted (the broken inference path means unhalting would have always-SHORT bias).
- next (operator): (1) read `docs/superpowers/plans/2026-05-08-ml-validation-broken.md`; (2) decide Bug 3 fix path — recommend 3.A restore regime augmentation in compute_normalized_features so inference matches training schema; (3) re-run per_pair_holdout_eval.py to get FIRST honest accuracy numbers; (4) only then retrain or unhalt. Bot stays halted until inference can actually run the model.
- honest accounting: I committed 30+ times tonight implying validated-via-holdout signal. Each "validated 70% M15" was wrong. The C1.A patch + Option G + primary_tf alignment + TF switch + heuristic bridges + news scaffolding are all REAL infrastructure work with standalone value; the holdout numbers attached to them were bogus. Load-bearing-question discipline working as intended — I trusted the holdout for hours before independently verifying it via direct probe.
...
- did: per "1 and 2 in parallel with skilled subagents", dispatched AI Engineer (full 17-pair retrain + validation, isolation:worktree, run_in_background) and Explore (H1 asymmetry investigation, read-only, run_in_background) in single message. **BOTH SUB-AGENTS DELIVERED WEAK RESULTS:**
  1. **Explore (a44111f618052ac02)** returned in 2 min with 5 tool uses, did NO greps or file reads, returned hand-waving plain-text speculation as final report rather than writing the requested doc to disk. Verified `docs/superpowers/plans/2026-05-07-track-C1-h1-asymmetry.md` did NOT exist post-agent-return. **Foreground-completed the investigation myself** — found the smoking gun in 4 greps + 1 read.
  2. **AI Engineer (a4f73113a71346d72)** returned in 6 min with 63 tool uses but final result was "Phase 2 + 3 scripts ready. Now I wait for monitor events" — agent set up retrain + ScheduleWakeup loop, but the wakeup never fired and the agent's session terminated before reporting. **Critical: agent's worktree was at `cf75ceb` — BEFORE the C1.A patch `e9528fa`** (worktree creation took an OLD ref, not current main). Its retrain ran on STALE code without the C1.A patch, producing None scalers as expected for the pre-fix code path. **Its "validation" was therefore measuring the OLD bug, not the patched state.** The 4 dirs with StandardScaler in worktree (AUD_USD, EUR_GBP, GBP_JPY, USD_CAD) all have mtime 18:33 = worktree-creation time (unchanged inherited artifacts); EUR_USD + USD_JPY were re-trained at 18:35 with old code → None scalers. Joint dir untouched. The full sweep effectively didn't happen.
- **H1 ASYMMETRY ROOT CAUSE (foreground investigation, doc at commit `72ea487`):** the holdout validator at `scripts/scheduled_retrain.py:451-453` hardcodes `lookahead=5` bars for ALL timeframes:
  ```python
  for i in range(min(len(features) - 5, 200)):
      future_close = test_df["close"].iloc[min(i + 5, len(test_df) - 1)]
  ```
  The trainer's lookahead is **24** (`buddy_training_helpers.py:536`). Model is trained to predict 24-bar-forward direction; holdout scores it against 5-bar-forward actual. **Train/eval label-horizon mismatch** — codebase grep shows trainer uses 24 (active path), feature-analysis tool uses 5 (analysis only); the holdout's 5 matches NOTHING in the actual training pipeline. Per-TF horizon ratios make the asymmetry quantitative: H1 (24h trained vs 5h actual = least correlated → 43%); M15 (6h trained vs 75min actual = partial → 54%); H4 (96h trained vs 20h actual = closer → 50.5%). **The 44% holdout that's been blocking promotion since Apr 16 has been measuring the wrong thing.** C1.A is structurally correct AND has measurably improved the model — the holdout protocol's lookahead=5 hardcode is what's blocking promotion now.
- commit: `72ea487 docs(plans): H1 asymmetry root cause`. Doc at `docs/superpowers/plans/2026-05-07-track-C1-h1-asymmetry.md` proposes Option G (replace `5` with `24` constant matching trainer; ~3 lines) and Option H (plumb lookahead through artifact metadata; ~20 LOC architectural).
- next: **OPERATOR DECISION** — three queued moves:
  1. **Apply Option G now (foreground)** — 3-line fix at `scripts/scheduled_retrain.py:451, 453` to use `lookahead=24` matching trainer. Re-run 2-pair smoke retrain; expect H1 to jump to 50-58% (the audit-CLEAN head's true ceiling at 24-bar horizon). If holdout passes, joint dir gets promoted automatically. **Recommend this.**
  2. **Re-dispatch full sweep on CORRECT main HEAD** — the prior AI Engineer ran on stale base. Worth doing AFTER Option G lands so the holdout gives meaningful numbers.
  3. **Clean up the AI Engineer worktree** (`/Users/buddy/Documents/ml_engine/.claude/worktrees/agent-a4f73113a71346d72`) — its uncommitted artifacts are based on stale code and should not be promoted; safe to `git worktree remove --force` after operator confirms.
- side: severe-overfitting signal in the AI Engineer's retrain log (gap=34.2% at epoch 8, model collapsed to memorizing) — possibly a real concern about data quality at 7500-candle window, BUT this was on stale code so may not reflect post-C1.A behavior. Defer until Option G smoke retrain.
- guardrails honored: halt=true throughout; no auto-unhalt; no LLM in runtime path; 52% threshold UNTOUCHED (the holdout BUG is what we're fixing, not the threshold); no push to origin; rollback backup intact; no destructive cleanup of stale worktree without operator confirmation.

### 2026-05-07 22:00 UTC Claude — C1.A patch shipped + smoke retrain (PARTIAL improvement, joint not promoted)
- did: per "proceed with the plan", completed C1 augmentation in foreground (loaded all 8 saved direction-model meta.pkl files; surveyed `direction_scaler.pkl` joblib presence across all 17 model dirs). **Concrete root cause traced:** `src/training/trainers/joint_trainer.py:391` calls `dir_trainer.train(..., skip_scaling=True, ...)` with comment "Data from load_direction_data is already scaled". `skip_scaling=True` triggers `transformer_trainer.py:474-476` which sets `self.scaler = None`. That nulls both `meta['scaler']` AND blocks the `direction_scaler.pkl` joblib write at `:1578`. 13 of 17 direction-model dirs (incl. joint, EUR_USD, GBP_USD, USD_JPY) had no scaler at inference; the 4 working dirs (AUD_USD, EUR_GBP, GBP_JPY, USD_CAD) had been retrained via the per-pair `cli/training.py:1944-1971` path which defaults `skip_scaling=False`. **Shipped C1.A Option F patch (commit `e9528fa`):** added `apply_scaler: bool = True` to `load_direction_data` (default preserves backward compat for `cli/candle_optimizer.py`, `scripts/train_meta_calibrator.py`, `tests/test_fold_debug.py`); `joint_trainer.py:175,504` now pass `apply_scaler=False`; `joint_trainer.py:391` flipped to `skip_scaling=False`; `transformer_trainer.py:1578` hardened from silent `if scaler is not None:` guard to a hard RuntimeError. Smoke-tested both branches in isolation: backward-compat default produces RobustScaler+clip (abs_max=10, scaler=RobustScaler); apply_scaler=False produces raw (abs_max=99.92, scaler=None). Backed up 48 pre-fix direction-model files to `trained_data/rollback/direction_pre_2026-05-07/` (commit `c985562`). Then ran `python scripts/scheduled_retrain.py --pairs EUR_USD,GBP_USD --granularity H1`: 226s wall-time, 2 pairs trained via `train_per_pair_correlation_ensemble` (per-pair correlation-transfer pipeline at `src/training/buddy_training_helpers.py`).
- **post-retrain SCALER STATE (verified on disk):**
  - EUR_USD: `meta['scaler']=StandardScaler` ✓ (mean≈0, scale≈1) AND `direction_scaler.pkl` present ✓ — **C1.A WORKED**
  - GBP_USD: scaler still None — pair acted as correlation DONOR, not retrained as target (mtime May 5 unchanged)
  - joint: scaler still None — joint dir not updated by per-pair pipeline (mtime Apr 28; only updated on promotion, which was HELD)
- **post-retrain HOLDOUT (multi-TF):**
  - H1: 43.2% (vs pre-fix 43.8% — essentially unchanged; FAILED 52% gate)
  - M15: **54.0%** (vs pre-fix 44.5% — **+9.5pp**; PASSED 52% gate)
  - H4: 50.5% (vs pre-fix 43.8% — +6.7pp; FAILED 52% gate)
  - aggregate: 49.2% (vs pre-fix ~44% — **+5pp on 1200 holdout samples**)
  - Promotion verdict: **HOLD** — candidate accuracy 0.4320 (primary TF=H1) < 0.5200 floor. Joint artifact NOT updated.
- **interpretation:** C1.A is structurally correct — the +9.5pp jump on M15 (cleanest signal) and +6.7pp on H4 are exactly what fixing a normalization mismatch produces. But H1 specifically did NOT benefit from the patch. Either (a) H1 has genuinely lower predictability than M15 at the current feature set — most likely; H1 is short-horizon noise vs M15's slightly longer signal; (b) there's a SECOND bug specific to the H1 holdout protocol that the patch didn't address; (c) the per-pair correlation-transfer pipeline mutates features differently per timeframe. The fact that aggregate went 44%→49.2% confirms structural improvement; promotion blocked by primary-TF gate is the constraint, not the fix.
- commit: `e9528fa` (C1.A patch), `c985562` (rollback backup), prior `4034df2` (root-cause doc), `5f78ae7` (augmentation findings).
- next: **OPERATOR DECISION** — three paths:
  1. **Run full 17-pair sweep** — more data may shift aggregate above 52%; cheap to run (~30 min), easy to roll back. Recommend.
  2. **Investigate H1-specific issue** — why M15+9.5pp but H1 ~0pp? Could be feature engineering (H1 uses different `lookahead`?), data quality (H1 has noisier returns at short horizon), or holdout protocol (H1 sample window differs). Read-only investigation, foreground.
  3. **Lower the 52% threshold** — explicitly forbidden by operator brief. Not an option.
  4. **Accept partial fix + close C1** — joint dir stays scaler-null; system halted; consumer wiring (heuristics) remains the path to keep system bridged. C1.A shipped on main; future retrains will benefit. Track D unblocking remains gated on H1 ≥52%.
- guardrails honored: halt=true throughout; no auto-unhalt; no LLM in runtime path; 52% threshold UNTOUCHED; rollback backup preserved; promotion HOLD respected (joint NOT updated since accuracy < floor); no push to origin.

...
def client(tmp_path, monkeypatch):
    ...
    # (StateEngine -> .claude/state.json on halt/unhalt) hit tmp_path, never the
    # repo's live operator state. Caught by the root-conftest pollution guard
    # (2026-07-18): these tests were flipping the real .claude/state.json halted
    # flag on every run — the exact Hard NO #2 surface the guard now protects.

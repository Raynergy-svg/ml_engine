...
export function CryptoMomentumPanel() {
  const { data, loading } = usePoll<CryptoMomentum>(
    "/api/crypto_momentum",
    5000,
  );
  ...
  if (loading && !data) {
    return (
      <Card className="p-3">
        <SectionTitle>Crypto Momentum</SectionTitle>
        <Loading />
      </Card>
    );
  }
  if (!data) {
    return (
      <Card className="p-3">
        <SectionTitle>Crypto Momentum</SectionTitle>
        <NotConnected label="Crypto momentum data unavailable" />
      </Card>
    );
  }
            ...
            <>
              ...
              {!lg.available && (
                <div className="py-2 font-mono text-[10.5px] text-faint">
                  ...
                  no broker path exists for crypto in this repo.
                </div>
              )}
            </>
          )}
        ...
        <div className="p-3">
          {!c ? (
            ...
            <>
              ...
              <Row
                label="Pre-registered OOS Sharpe"
                value={c.pre_registered_oos_sharpe.toFixed(2)}
              />
              <Row
          ...
          <div className="mt-2 border-t pt-2 font-mono text-[10px] text-faint hairline">
            This signal did NOT clear the crypto ship gate (significance fails
            at N=15 and N=3). This lane exists to let a genuine live-forward
            record — not another backtest — answer whether the +0.75
            pre-registered OOS Sharpe holds up.
          </div>
        </div>
      </Card>

...
export function CryptoMomentumPanel() {
  const { data, loading } = usePoll<CryptoMomentum>("/api/crypto_momentum", 5000);
  const { data: lanes } = usePoll<LaneStatus>("/api/lanes", 4000);
  const laneHalted = lanes?.lanes?.crypto_momentum ?? null;

  if (loading && !data) {
    return <Card className="p-3"><SectionTitle>Crypto Momentum</SectionTitle><Loading /></Card>;
  }
  if (!data) {
    return <Card className="p-3"><SectionTitle>Crypto Momentum</SectionTitle><NotConnected label="Crypto momentum data unavailable" /></Card>;
  }
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="flex flex-col">
        <SectionTitle right={
          ...
          Crypto Momentum · H2/H4 XS-momentum, shadow lane
        </SectionTitle>
        <div className="p-3">
          {!data.has_run ? (
            <NotConnected
              label="No shadow cycle recorded yet"
              hint="Lane is built and gated but has not run a cycle in production — this is the honest starting state, not a fault. Run scripts/run_crypto_momentum_shadow.py to start accumulating a forward record."
            />
          ) : (
            <>
              ...
              {!lg.available && (
                <div className="py-2 font-mono text-[10.5px] text-faint">
                  No live_gate_state.json on disk — lane has never been armed; no broker path exists for crypto in this repo.
                </div>
              )}
            </>
          )}
        ...
        <div className="p-3">
          {!c ? (
            ...
            <>
              ...
              <Row label="Pre-registered OOS Sharpe" value={c.pre_registered_oos_sharpe.toFixed(2)} />
          ...
          <div className="mt-2 border-t pt-2 font-mono text-[10px] text-faint hairline">
            This signal did NOT clear the crypto ship gate (significance fails at N=15 and N=3).
            This lane exists to let a genuine live-forward record — not another backtest —
            answer whether the +0.75 pre-registered OOS Sharpe holds up.
          </div>
        </div>
      </Card>
      ...
      <Card className="flex flex-col">
        ...
        <div className="flex gap-4 p-3">
          <BookSide title="Longs" weights={data.current_book?.longs ?? {}} color="#2bd17e" />
          <BookSide title="Shorts" weights={data.current_book?.shorts ?? {}} color="#ff4d6d" />
        </div>
      </Card>

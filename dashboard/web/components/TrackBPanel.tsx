...
export function TrackBPanel() {
  ...
  if (loading && !data) {
    return (
      <Card className="p-3">
        <SectionTitle>Track B — Filing-Text Research Alpha</SectionTitle>
        <Loading />
      </Card>
    );
  }
  if (!data) {
    return (
      <Card className="p-3">
        <SectionTitle>Track B — Filing-Text Research Alpha</SectionTitle>
        <NotConnected label="Track B data unavailable" />
      </Card>
    );
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="flex flex-col">
        ...
        <div className="p-3">
          <Row
            label="Scored filings (post-cutoff)"
            value={
              <span
                className={
                  data.n_scored_filings != null &&
                  c &&
                  data.n_scored_filings < c.power_required_n_filings
                    ? "text-warn"
                    : ""
                }
              >
                {data.n_scored_filings ?? "—"}{" "}
                {c ? `/ ~${c.power_required_n_filings} needed` : ""}
              </span>
            }
          />
          {!data.has_run ? (
            ...
            <>
              ...
              {!lg.available && (
                <div className="py-2 font-mono text-[10.5px] text-faint">
                  ...
                  the frozen harness&apos;s own verdict for this signal is
                  INSUFFICIENT, not REAL.
                </div>
              )}
            </>
          )}
          ...
          {!c ? (
            ...
            <>
              ...
              <Row
                ...
                value={`quality ${c.composite_weights.fundamental_quality} / red-flags ${c.composite_weights.accounting_red_flags} / outlook ${c.composite_weights.forward_outlook}`}
              />
              <Row label="Book" value={c.book} mono={false} />
              <Row
                label="Rebalance"
                value={`every ${c.rebalance_step_days}d (held between scoring batches)`}
              />
              <Row
              ...
              <div className="mt-2 border-t pt-2 font-mono text-[10px] text-faint hairline">
                Sources: {c.source_prereg_doc} · {c.source_scaleup_doc} ·{" "}
                {c.source_adversarial_review_doc}
              </div>
            </>
          )}
        </div>
      ...
      <Card className="flex flex-col">
        ...
        <div className="flex gap-4 p-3">
          <BookSide
            title="Longs (Q5, equal-weight)"
            weights={data.current_book?.longs ?? {}}
            color="#2bd17e"
          />
          <BookSide
            title="Shorts (none — long-only book)"
              ...
              {data.recent_cycles.map((cy) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-faint">
                    {cy.n_longs}L ({cy.n_scored_filings} scored)
                  </span>
                  <span

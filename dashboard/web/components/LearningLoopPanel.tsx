...
export function LearningLoopPanel() {
  ...
  if (loading && !data) {
    return (
      <Card className="p-3">
        <SectionTitle>Continual Learning</SectionTitle>
        <Loading />
      </Card>
    );
  }
  if (!data) {
    return (
      <Card className="p-3">
        <SectionTitle>Continual Learning</SectionTitle>
        <NotConnected label="Learning loop data unavailable" />
      </Card>
    );
  }
  ...
  const metricsUnavailable =
    last?.decision === "no_new_data"
      ? "not evaluated — no new holdout data"
      : "not reported by latest cycle";
    ...
    <div className="grid gap-4 lg:grid-cols-2">
          ...
          right={
            <span className="flex items-center gap-1.5">
              <Badge color="#5a6677" dot>
                {data.consumed_by_live_sizing
                  ? "LIVE SIZING"
                  : "SHADOW — advisory only"}
              </Badge>
              <Badge
                color={data.pending_retrain_markers > 0 ? "#f5b14c" : "#2bd17e"}
                dot
              >
                {data.pending_retrain_markers} MARKER
                {data.pending_retrain_markers === 1 ? "" : "S"} PENDING
              </Badge>
              <Badge
                color={data.has_run ? decisionColor(last?.decision) : "#5a6677"}
                dot
              >
                {data.has_run
                  ? (last?.decision ?? "—").toUpperCase()
                  : "NOT YET RUN"}
              </Badge>
            </span>
          }
          ...
          {!data.has_run ? (
            ...
            <div className="font-mono text-[12px]">
              <div className="flex items-center justify-between border-t py-1.5 hairline first:border-t-0">
                ...
                <span className="text-text">{last?.decision}</span>
              </div>
              <div className="flex items-center justify-between border-t py-1.5 hairline">
                ...
                <span className="text-text tnum">
                  {last?.incumbent_brier != null && last.candidate_brier != null
                    ? `${last.incumbent_brier.toFixed(4)} → ${last.candidate_brier.toFixed(4)}`
                    : metricsUnavailable}
                </span>
              </div>
              <div className="flex items-center justify-between border-t py-1.5 hairline">
                ...
                <span className="text-text tnum">
                  {last?.max_size_multiplier != null
                    ? `${last.max_size_multiplier.toFixed(2)} (≤ 1.0 always)`
                    : metricsUnavailable}
                </span>
              </div>
              <div className="flex items-center justify-between border-t py-1.5 hairline">
                <span className="text-faint">
                  markers drained / new outcomes
                </span>
                <span className="text-text tnum">
                  {last?.markers_drained ?? 0} / {last?.new_outcomes ?? 0}
                </span>
              </div>
              <div className="flex items-center justify-between border-t py-1.5 hairline">
                <span className="text-faint">time</span>
                <span className="text-text tnum">
                  {shortTime(last?.timestamp)}
                </span>
              </div>
      ...
      <Card className="flex flex-col">
        <SectionTitle
          right={
            <Badge color="#5a6677" dot>
              {data.recent_cycles?.length ?? 0} CYCLES
            </Badge>
          }
        >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {c.candidate_brier != null && c.incumbent_brier != null && (
                      <span className="text-faint">
                        {" "}
                        · brier {c.incumbent_brier.toFixed(4)}→
                        {c.candidate_brier.toFixed(4)}
                      </span>
                    )}
                    <div className="text-[10px] text-faint">
                      {c.markers_drained ?? 0} marker(s) drained ·{" "}
                      {c.new_outcomes ?? 0} new outcome(s)
                    </div>
                  </div>
                  <span className="shrink-0 text-[10px] text-faint tnum">
                    {shortTime(c.timestamp)}
                  </span>
                </div>
              ))}
            </div>

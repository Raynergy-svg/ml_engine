...
export function CryptoCarryPanel() {
  ...
  if (loading && !data) {
    return <Card className="p-3"><SectionTitle>Crypto Carry</SectionTitle><Loading /></Card>;
  }
  if (!data) {
    return <Card className="p-3"><SectionTitle>Crypto Carry</SectionTitle><NotConnected label="Crypto carry data unavailable" /></Card>;
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="flex flex-col">
        <SectionTitle right={
          <span className="flex items-center gap-1.5">
            ...
            <Badge color="#f5b14c" dot>RISK PREMIUM · LIQUIDATION TAIL</Badge>
          </span>
        }>
            ...
            <>
              ...
              {!lg.available && (
                <div className="py-2 font-mono text-[10.5px] text-faint">
                  No live_gate_state.json on disk — lane has never been armed; no broker/exchange path exists for crypto in this repo.
                </div>
              )}
            </>
          )}
      ...
      <Card className="flex flex-col">
        <SectionTitle right={<Badge color="#f5b14c" dot>PRE-REGISTERED, GATE-FAIL</Badge>}>
        ...
        <div className="p-3">
          {!c ? (
            ...
            <>
              ...
              <Row label="Funding lookback" value={`${c.lookback_d}d trailing mean`} />
              ...
              <Row label="Cost (round-trip, 2 legs)" value={`${c.cost_bps_roundtrip} bps`} />
          ...
          <div className="mt-2 space-y-1.5 border-t pt-2 font-mono text-[10px] text-faint hairline">
            <p>
              This construction did NOT clear the ship gate (turnover cost dominates a real
              funding premium — see source doc §5). This lane exists to let a genuine
              live-forward record — not another backtest — answer whether a real deployment
              fares differently.
            </p>
            <p className="text-warn">
              RISK PREMIUM, NOT FREE MONEY: cash-and-carry harvests a real, market-neutral
              funding premium but carries a real exchange-solvency / liquidation tail (basis
              blowouts, counterparty failure). Neither this backtest nor this shadow ledger
              models that tail. {data.risk_premium_note ?? ""}
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card className="flex flex-col">
        ...
        <div className="flex gap-4 p-3">
          <BookSide title="Long carry (spot / short perp)" weights={data.current_book?.longs ?? {}} color="#2bd17e" />
          <BookSide title="Reverse carry (unused)" weights={data.current_book?.shorts ?? {}} color="#ff4d6d" />
        </div>
      </Card>
            ...
            <div className="font-mono text-[11px]">
              {data.recent_cycles.map((cy) => (
                <div key={cy.forward_cycle_seq} className="flex items-center gap-2 border-t py-1.5 hairline first:border-t-0">
                  ...
                  <span className="shrink-0 text-faint">{cy.n_longs}L/{cy.n_shorts}S</span>

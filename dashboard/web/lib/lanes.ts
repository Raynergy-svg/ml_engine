...
export const LANE_META: Record<string, { label: string; hint: string }> = {
  ...
  crypto_carry: { label: "Crypto carry", hint: "cash-and-carry (funding) shadow lane — risk premium with a liquidation tail, no LiveGate arm exists" },
};

export const LANE_ORDER = ["oanda_fx", "equity", "brain", "crypto_momentum", "track_b", "crypto_carry"] as const;

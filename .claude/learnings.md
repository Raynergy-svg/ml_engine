...
- [2026-03-18] **architecture/lazy_imports**: Eager imports in src/scanner/__init__.py caused cascading failures — importing ANY automation module pulled in rich/tensorflow. Fix: lazy __getattr__ pattern. All 40+ symbols still accessible but only load when accessed. Pattern: always use lazy imports in package __init__.py when submodules have heavy deps.
- [2026-03-18] **feature/accuracy_gate**: Built AccuracyGate (src/scanner/automation/accuracy_gate.py) — auto-blocks pairs below 55% directional accuracy after 5 trades. Integrated into LearningEngine.analyze_trade() and ContinuousScanner learning loop. Pairs auto-unblock if retrained model improves accuracy. Replaces manual blocked_pairs management.
- [2026-03-18] **resilience/oanda_retry**: Added exponential backoff (1s→2s→4s, 3 retries) to OANDA candle fetching in training pipeline. Auth errors (401/403) fail fast. Per-pair isolation means one pair's failure doesn't block others. Addresses the previous "5/7 pairs failed" incident (which was actually stale state — all 7 succeeded on latest retrain).
- [2026-03-18] **stale_state/retrain_status**: state.json said "5 of 7 pairs failed retraining" but retrain_all_summary.json showed ALL 7 succeeded (trained 11 hours later). Pattern: always verify claims in state.json against source-of-truth files before acting on them.

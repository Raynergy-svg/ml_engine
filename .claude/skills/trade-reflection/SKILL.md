...
- Append **one line** to `.claude/learnings.md` in the exact format:
...
1. **Append learning** to `.claude/learnings.md` (always do this, same format as lightweight)
2. **Propose weight delta** to `.claude/proposed_weights.json` if one agent's
...
3. **Propose config adjustment** by appending to `.claude/config_adjustments.json`
...
4. **Draft a rule** for `.claude/rules/trading.md` — ONLY if you have strong
...
- You MAY write to: `.claude/learnings.md`, `.claude/rules/*.md`,
  `.claude/config_adjustments.json`, `.claude/proposed_weights.json`,
  `.claude/state.json` (only the `improvement_focus` field)

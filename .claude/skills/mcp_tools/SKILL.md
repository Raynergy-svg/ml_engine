...
Register in Claude Desktop `mcp.json`:
```json
{
  "mcpServers": {
    "buddy": {
...
### 7. `get_learnings` -- Read .claude/learnings.md
...
```json
{"diagnostics": {"status": "DEGRADED"}, "heal": {"status": "applied", "actions_taken": [{"action": "reduce_risk_per_trade_pct", "success": true}], "files_modified": [".claude/config_adjustments.json"], "degraded_mode": false}}
```
...
```json
{"status": "appended", "path": ".claude/learnings.md"}
```

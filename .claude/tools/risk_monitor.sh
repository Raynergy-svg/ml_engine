...
halted="?"; mode="?"
oanda_fx_halted="?"; equity_halted="?"; brain_halted="?"; crypto_momentum_halted="?"; track_b_halted="?"; crypto_carry_halted="?"
if [ ! -r "$STATE" ]; then
...
PYEOF
    ...
    if [ "$lane_report" = "ERR" ] || [ -z "$lane_report" ]; then
      ...
      crypto_carry_halted="$(printf '%s\n' "$lane_fields" | grep -oE 'crypto_carry=[^ ]+' | cut -d= -f2)"
...
lane_status="oanda_fx_halted=${oanda_fx_halted} equity_halted=${equity_halted} brain_halted=${brain_halted} crypto_momentum_halted=${crypto_momentum_halted} track_b_halted=${track_b_halted} crypto_carry_halted=${crypto_carry_halted}"
if [ ${#alarms[@]} -eq 0 ]; then

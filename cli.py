"""
Enhanced CLI for stock market prediction model with online data fetching capabilities.
Provides a user-friendly interface for model training, prediction, and analysis.
"""
...
class StockPredictionCLI:
    """
    Enhanced Command Line Interface for stock market prediction with online data fetching.
    """
    ...
    def initialize_engines(self):
        ...
        self.memory_manager = MemoryManager(
            ...
            threshold_mb=self.config.get("memory", {}).get("threshold_mb", 1000),
            log_level=self.config.get("memory", {}).get("log_level", "INFO"),
            proactive_cleanup=self.config.get("memory", {}).get("proactive_cleanup", True)
        )
        ...
        self.integrator = NeuralNetworkIntegrator(self.config.get("integrator", {}))
    ...
    async def fetch_market_indicators(
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(indicators)} market indicators")
            return data
        except Exception as e:
    ...
    async def fetch_economic_data(
        ...
        try:
            ...
            logger.info(f"Successfully fetched economic data")
            return data
        except Exception as e:
    ...
    async def prepare_data_for_prediction(
        ...
        start_date = (datetime.datetime.now() - datetime.timedelta(days=days_back + 10)).strftime("%Y-%m-%d")
        ...
        economic_data = results[2] if include_market_data and include_economic_data else results[1] if include_economic_data else pd.DataFrame()
